...
window.ovrtWinDetailed = function(details) {
    window.ovrt.completeWinDetails(details)
}
window.ovrtWinTitles = function(titles) {
    console.log('Got titles proper');
    window.ovrt.completeWindowTitles(titles)
}
window.ovrtMonitorTotal = function(total) {
    window.ovrt.totalMonitors = total
}

let titleTimeout = -1
let titleTimeoutInterval = 5000
function requestIntervalWindowTitles() {
    if (typeof window.GetWindowTitles === 'function')
        window.GetWindowTitles('completeIntervalWinTitles')
}
function completeIntervalWinTitles(titles) {
    console.log('Titles updated');
    window.ovrt.winTitles = titles;
    window.ovrt.onWinTitlesUpdated(titles)
}

let fingersTimeout = -1
let fingerTimeoutInterval = 1000
function requestFingerCurls() {
    window.GetFingerCurls('completeFingerCurls')
}
function completeFingerCurls(curls) {
    window.ovrt.fingerCurls = curls
}

function DevicePositionUpdate(deviceInfo) {
    if (window.ovrt.updateDeviceInfo)
        window.ovrt.deviceInfo = deviceInfo
}
function OverlayTransformChanged(updateData) {
    if (window.ovrt.updateWindows)
        window.ovrt.onWinTransformChanged(updateData)
}
function InteractionStateChanged(isInteracting) {
    window.ovrt.onWinInteractionChanged(isInteracting)
}
function ReceiveMessage(message) {
    console.info('got message');
    window.ovrt.onMessageReceived(message)
}
function OverlayOpened(uid) {
    window.ovrt.completeWinSpawn(uid)
}
function OverlayClosed(uid) {
    window.ovrt.onWinClosed(uid)
}
...
window.ovrt = {
    ...
    onWinTransformChanged: function(transformUpdate) {
        console.log('WinTransformChanged', transformUpdate)
    },
    onWinOpened: function(uid) {
        console.log('LocalWindowOpened', uid)
    },
    onWinClosed: function(uid) {
        console.log('WindowClosed', uid)
    },
    // onWinInteractionChanged: function (isInteracting) { console.log('WinInteractionChanged', isInteracting) },
    onMessageReceived: function(message) {
        console.log('MessageReceived', message)
    },
    onWinTitlesUpdated: function(titles) {
        console.log('WinTitlesUpdated', titles)
    },
    onLog: function(logData) {
        console.info('ConsoleLog', logData)
    },
    onLogError: function(errorData) {
        console.info('LoggedError', errorData)
    },
   ...
   */
    completeWinSpawn: function(uid) {
        ...
        if (typeof winData.contents === 'object')
            normalizedContents = JSON.stringify(winData.contents)
        else
            normalizedContents = String(normalizedContents)
        let stringContents = JSON.stringify(winData.contents)
        window.SetContents(String(uid), Number(winData.type), normalizedContents)
        windowData.uid = uid
        if (typeof winData.callback === 'function')
            winData.callback(uid, winData.data)
        this.onWinOpened()
    },
   ...
   */
    completeWinDetails: function(winTransform) {
        ...
        if (typeof winData.callback === 'function')
            winData.callback(winTransform, winData.data)
    },
   ...
   */
    completeWindowTitles: function(titles) {
        ...
        if (typeof titleData.callback === 'function')
            titleData.callback(titles, titleData.data)
    },
   ...
   */
    setFingerUpdateFlag: function(enable) {
        ...
        } else if (!this.updateFingers)
            clearInterval(window.fingersTimeout)
    },
   ...
   */
    setTitlesUpdateFlag: function(enable) {
        ...
        } else if (!this.updateTitles)
            clearInterval(window.titleTimeout)
    },
